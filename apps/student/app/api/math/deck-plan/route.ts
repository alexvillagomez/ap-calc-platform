/**
 * POST /api/math/deck-plan — math twin of /api/mcat/deck-plan.
 *
 * Returns the ORDERED in_depth keyword decks for a scope so the flashcard walk
 * plays decks in curriculum order and glosses over already-mastered ones.
 *
 * Scope (most specific wins):
 *   keyword_id (+ category_id)  → that keyword's deck
...
export async function POST(request: Request) {
  ...
  if (Array.isArray(body.category_ids) && body.category_ids.length > 0) {
    ...
    // Whole-course stream: this course's categories in order_index order.
  ...
  const [keywords, statesRes, fcRes, catRes] = await Promise.all([
    loadTargetKeywords(supabase, categoryIds, course),
    supabase
    ...
    supabase
      ...
      .eq("status", "active"),
    supabase.from("math_categories").select("id, label"),
  ]);
