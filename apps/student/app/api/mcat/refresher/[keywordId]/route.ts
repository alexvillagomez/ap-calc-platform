/**
 * GET /api/mcat/refresher/[keywordId]
 *
 * Returns a SHORT refresher ({ rule_latex, example_latex }) for an mcat keyword.
 * Cache-first against mcat_refreshers; on miss, generate via OpenAI, store, return.
 *
 * FAIL-SOFT: if the mcat_refreshers table is missing (migration not yet applied)
...
export async function GET(
  ...
  // 3. Generate + store.
  try {
    const generated = await generateAndStoreRefresher(supabase, "mcat", kw);
    if (!generated) {
      return NextResponse.json(
        {
          rule_latex: null,
    ...
    console.error(`mcat refresher generation failed for ${keywordId}:`, detail);
